function [R, Z, T, eig, count, sufbot] = traceray_circ(theta_deg, env, para,R_stop)
%   Traces the ray corresponding to a particular take-off angle when the
%   sound speed gradient is non-zero.
...
N            = para.N;
if N<100000, N = 100000;end;
countlength  = para.countlength;
bottom_stop  = para.bottom_stop;
if isempty(bottom_stop); bottom_stop =0;end;
surface_stop  = para.surface_stop;
...
dir=sign(theta_deg); p_current=cos(theta_0)./c(z==z_source);
    ...
    for n=1: N % Counter along the ray
        ...
        if dir==1; % Down going wave
           ...
           if k>k_bottom; %Bottom detected;
                ...
                theta_in=acos(p_current*c_bottom);
                theta_out=theta_in+2*angle_current;
                theta_current=theta_out;
                ...
                if theta_current<=0 % continue down going
                    ...
                    newibathy=ibathy;
                    while x_temp+delta_x>=R_bathy(newibathy+1)
                        newibathy=newibathy+1;
                    end;
                ...
                if k<length(c)
                    w1=sqrt(abs(1-(p_current*c(k))^2));
                    w2=sqrt(1-(p_current*c(k+1))^2);
                    temp1=c(k+1)*(1+w1); temp2=c(k)*(1+w2);
                    g=(c(k+1)-c(k))/(z(k+1)-z(k));
                    if  p_current*c(k+1)<1 % No turning in next layer below
                        delta_x=(w1-w2)/(p_current*g+eps);
                        delta_z=del_z;
                        delta_t=abs(1/(g+eps)*log(temp1/temp2));
                        k=k+1;
                    else % Turning point within the next layer below
                        turning_below=turning_below+1; dir=-dir;
                        delta_x=2*abs(w1/(p_current*g+eps));
                        temp2=c(k);
                        delta_t=2*abs(1/(g+eps)*log(temp1/temp2));
                        delta_z=0;
            ...
            if k==1 % surface hit
                ...
                if surface_stop>0;
                    if surface_hits>surface_stop-1; break; end;
                end
            ...
            elseif z_temp>=depth_current; %Bottom detected
                ...
                if p_current*c(k+1)>=1 || p_current*c(k)>=1
                    ...
                    theta_current=acos(p_current*c(k+1));
                ...
                k=k-1;
                g=(c(k+1)-c(k))/(z(k+1)-z(k));
                w1=sqrt(1-(p_current*c(k))^2);
                w2=sqrt(abs(1-(p_current*c(k+1))^2));
                if p_current*c(k)<1 % No turning within next layer above
                    temp1=c(k+1)*(1+w1);
                    temp2=c(k)*(1+w2);
                    delta_x=(w1-w2)/(p_current*g+eps);
                    delta_t=abs((1/(g+eps))*log(temp1/(temp2+eps)));
                    delta_z=-del_z;
                else
                    ...
                    temp2=c(k)*(1+w2);
                    delta_z=0;
                    delta_x=abs(2*w2/(p_current*g+eps));
                    delta_t=2*abs((1/(g+eps))*log(temp1/(temp2+eps)));
                    k= k+1;
                end; % End turning point
        ...
        if k==k_target
            ...
            eig_angle(target_count)=dir*acos(p_current*c(k_target));
...
para.countlength=20000000;
if bottom_hits>para.countlength; disp ('Too many bottom hits'); end;
