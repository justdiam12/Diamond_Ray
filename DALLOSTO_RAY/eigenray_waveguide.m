...
%3/5/2008
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% theta = [thtDP thtSP thtBP];
% RANGERES = .2;
% DPTHRES = .01;
% clf
function [Range,Depth,time, launch_angle] = eigenray_waveguide(theta,RANGERES,DPTHRES,RR,RD,SD,dcz,cw,zw,cb,zb)
%%%%%%%%%%%%%%%%
% INPUTS
...
t = 0;
dtht = .1;
%EigenRay While Loop to fit within DPTHRES  
RayOFF = sqrt((D(end)-RD).^2+(R(end)-RR).^2);

while RayOFF > DPTHRES
    
    %Probably better to compute a new set of rays, bracketing first
 for ndraw = 1:3
    
    if ndraw == 1
thetalaunch = -theta(ntheta);
    elseif  ndraw == 2
thetalaunch = -theta(ntheta)-dtht;
    elseif ndraw == 3
thetalaunch = -theta(ntheta)+dtht;
    end
...
R(Rminus+1:end) = RR;
%Fix end time for these points;
t(Rminus+1:end) = t(Rminus)+sqrt((D(Rminus+1:end)-D(Rminus)).^2+(R(Rminus+1:end)-R(Rminus)).^2)/cw(ztemp);
...
% 
%       
plot(RR-R,D,'Linewidth',2)
hold on
plot(R-5,D*0+zw(end),'k','Linewidth',2)
plot(R-5,D*0,'k','Linewidth',2)

set(gca,'Ydir','reverse','Fontsize',14)
xlabel('RANGE (m)')
ylabel('DEPTH (m)')
plot(0,RD,'ko','Linewidth',2,'Markerface','r','Markersize',11)
plot(RR,SD,'ko','Linewidth',2,'Markerface','b','Markersize',11)
title(theta)
drawnow
...
[RayOFF,ind] = (min(abs(RD-[D1(end) D2(end) D3(end)])));

    if ind == 1
    dtht = dtht/2;
    elseif  ind == 2
theta(ntheta) = theta(ntheta)+dtht;
    elseif ind == 3
theta(ntheta) = theta(ntheta)-dtht;
    end
...
launch_angle(ntheta) = theta(ntheta);
