C This is the Rough Surface RAM made by J.W.CHOI on 11 March 2009
C Water Density Profile given in the input file was used.
C The rough sea surface interface is assumed as internal interface in the water medium


c Return-Path: <collins@noddy.nrl.navy.mil>

c Date: Fri, 03 Oct 2003 13:28:16 -0400
c From: Mike Collins <collins@noddy.nrl.navy.mil>



c Peter,

c Here are the codes. I highly recommend that you give the graphics codes a
c try. They are completely self-contained. All you need is a FORTRAN compiler and
c Postscript display or printer to use them. I hope they turn out to be of some use.
...
c     Research Centre. 
c
      complex ci,ksq,ksqb,u,v,r1,r2,r3,s1,s2,s3,pd1,pd2,tlg,upg,dng,
     >   tlline,ksqw
      real k0
c
c     mr=bathymetry points, mz=depth grid, mp=pade terms.
c
      parameter (mr=60000,mz=60000,mp=12)
      dimension rb(mr),zb(mr),cw(mz),cb(mz),rhob(mz),attn(mz),alpw(mz),
     >   alpb(mz),f1(mz),f2(mz),f3(mz),ksq(mz),ksqw(mz),ksqb(mz),u(mz),
     >   v(mz),tlg(mz),upg(mz),dng(mz),r1(mz,mp),r2(mz,mp),r3(mz,mp),
     >   s1(mz,mp),s2(mz,mp),s3(mz,mp),pd1(mp),pd2(mp),rhow(mz),
     >   attnw(mz)
c
c      open(unit=1,status='old',file='ram_rsfc2.in')
c      open(unit=2,file='ram_rsfc2.line')
c      open(unit=3,file='ram_rsfc2.grid')

	open(unit=1,status='old',file='WVGDinput.in')
      open(2,file='PEoutput.line')
      open(3,file='PEoutput.grid')
	open(4,file='UPoutput.grid')
	open(5,file='DNoutput.grid')
	open(6,file='LFoutput.grid')
	open(7,file='RToutput.grid')
...
c
     ...
     >   pd1,pd2,tlg,upg,dng,rhow,attnw)
c
...
c
    1 call updat(mr,mz,nz,mp,np,iz,ib,dr,dz,eta,omega,rmax,c0,k0,ci,r,
     ...
     >   r1,r2,r3,s1,s2,s3,pd1,pd2,rhow,attnw)
      call solve(mz,nz,mp,np,iz,u,v,r1,r2,r3,s1,s2,s3)
      r=r+dr
      call outpt(mz,mdr,ndr,ndz,iz,nzplt,lz,ir,dir,eps,r,f3,u,v,tlg,dr,
     >   k0,upg,dng)
...
c
     ...
     >   alpw,alpb,ksqw,ksqb,rhow,attnw)
      call selfs(mz,nz,mp,np,ns,iz,zs,dr,dz,pi,c0,k0,rhob,alpw,alpb,ksq,
     >   ksqw,ksqb,f1,f2,f3,u,v,r1,r2,r3,s1,s2,s3,pd1,pd2,rhow)
      call outpt(mz,mdr,ndr,ndz,iz,nzplt,lz,ir,dir,eps,r,f3,u,v,tlg,dr,
     >   k0,upg,dng)
c
...
c
     ...
     >   f1,f2,f3,r1,r2,r3,s1,s2,s3,pd1,pd2,rhow)
c
      return
      end
c
...
c
     ...
     >   attn,alpw,alpb,ksqw,ksqb,rhow,attnw)
...
c
    1 do 2 i=1,nz+2
      ksqw(i)=((omega/cw(i))*(1.0+ci*eta*attnw(i)))**2-k0**2
...
c
     ...
     >   ksqb,f1,f2,f3,r1,r2,r3,s1,s2,s3,pd1,pd2,rhow)
      complex d1,d2,d3,rfact,ksq(mz),ksqb(mz),r1(mz,mp),r2(mz,mp),
     >   r3(mz,mp),s1(mz,mp),s2(mz,mp),s3(mz,mp),pd1(mp),pd2(mp),
     >   ksqw(mz)
      real k0,rhob(mz),f1(mz),f2(mz),f3(mz),alpw(mz),alpb(mz),rhow(mz)
     >   
c
      a1=k0**2/6.0
      a2=2.0*k0**2/3.0
...
c
     ...
     >   f3,r1,r2,r3,s1,s2,s3,pd1,pd2,rhow,attnw)
      complex ci,ksq(mz),ksqb(mz),r1(mz,mp),r2(mz,mp),r3(mz,mp),
     >   s1(mz,mp),s2(mz,mp),s3(mz,mp),pd1(mp),pd2(mp),ksqw(mz)
      real k0,rb(mr),zb(mr),attn(mz),cb(mz),rhob(mz),cw(mz),f1(mz),
     >   f2(mz),f3(mz),alpw(mz),alpb(mz),rhow(mz),attnw(mz)
c
...
c
     ...
     >   ksqw,ksqb,f1,f2,f3,r1,r2,r3,s1,s2,s3,pd1,pd2,rhow)
c
c     Varying profiles.
c
      if(r.ge.rp)then
...
c
     ...
     >   alpb,ksq,ksqw,ksqb,f1,f2,f3,u,v,r1,r2,r3,s1,s2,s3,pd1,pd2,rhow)
      complex u(mz),v(mz),ksq(mz),ksqb(mz),r1(mz,mp),r2(mz,mp),
     >   r3(mz,mp),s1(mz,mp),s2(mz,mp),s3(mz,mp),pd1(mp),pd2(mp),
     >   ksqw(mz)
      real k0,rhob(mz),alpw(mz),alpb(mz),f1(mz),f2(mz),f3(mz),rhow(mz)
     >   
c
...
c
      ...
      subroutine outpt(mz,mdr,ndr,ndz,iz,nzplt,lz,ir,dir,eps,r,f3,u,v,
     >   tlg,dr,k0,upg,dng)
      complex ur,up,dn,lf,rt,u(mz),v(mz),tlg(mz),upg(mz),dng(mz)
      real k0,f3(mz)
c
...
c
	...
	dn=u(q)*f3(q)
	j=j+1
c      tlg(j)=-20.0*alog10(cabs(ur)+eps)+10.0*alog10(r+eps)   Transmission loss in dB
c		an attempt to output particle velocities (u and v) comment out below and add
c	kcmplx = cmplx(k0,0.0)
c      rmod = exp(cmplx(0.0,1.0)*kcmplx*r+atan(1.0))
	tlg(j)= (ur+eps)/sqrt(r+eps)
	upg(j) = (up+eps)/sqrt(r+eps)
	dng(j) = (dn+eps)/sqrt(r+eps)
...
c      write(3)(tlg(j),j=1,lz)
      ...
      mdr=0
	do j=1,lz
      write(3,*)	r, real(tlg(j)), aimag(tlg(j))
	write(4,*)	real(upg(j)), aimag(upg(j))
	write(5,*)	real(dng(j)), aimag(dng(j))
	enddo
	else if(mdr.eq.ndr-1)then
	do j=1,lz
	write(6,*)	real(tlg(j)), aimag(tlg(j))
	enddo
	else if(mdr.eq.1)then
	do j=1,lz
	write(7,*)	real(tlg(j)), aimag(tlg(j))
	enddo
      end if
