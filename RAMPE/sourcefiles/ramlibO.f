...
c     Unit 1, 'ram.in', is the input: a text file as specified in the User's Guide.
...
c
     ...
     >   dir,dr,dz,pi,eta,eps,omega,rmax,c0,k0,r,rp,rs,rb,zb,cw,
...
c
c     Unit 2, 'tl.line', is a text output file that will get the dB transmission loss
c     versus range (2 columns) at the target depth.
      open(unit=2,status='unknown',file=linefile)
c
c     Unit 3, 'tl.grid', is a Fortran unformatted file that will get the dB
c     transmission loss on a grid of ranges and depths.
...
c use "self starter"
         x0 = 0.0
         call selfs(mz,nz,mp,np,ns,iz,zs,dr,dz,pi,c0,k0,rhow,rhob,alpw,
...
c 
     ...
     >                    theta0, dtheta, u)
      call matrc(mz,nz,mp,np,iz,jz,dz,k0,rhow,rhob,alpw,
...
c
     ...
     >   k0,cw(mz),cb(mz),rhob(mz),attn(mz),alpw(mz),alpb(mz),
     >   rhow(mz),attnw(mz)
c     Locals
      real(ram_real) :: tempkw
      complex(ram_complex) :: tempkb
...
c
    ...
    4 continue
        ...
        if(i-j.eq.1)go to 6
        dprof = (prof(i)-prof(j))/(i-j)
        do 5 k=j+1,i-1
          prof(k)=prof(j) + (k-j)*dprof
    5   continue
    6   j=i
...
c
     ...
     > bind(c)
      ...
      complex(ram_complex) :: 
     >   d1,d2,d3,rfact,ksq(mz),ksqb(mz),r1(mz,mp),r2(mz,mp),
     >   r3(mz,mp),s1(mz,mp),s2(mz,mp),s3(mz,mp),pd1(mp),pd2(mp),
     >   ksqw(mz)
      real(ram_real) :: 
     >   dz,k0,rhob(mz),f1(mz),f2(mz),f3(mz),alpw(mz),alpb(mz),
     >   rhow(mz)
c
      a1=k0**2/6.0
      a2=2.0*k0**2/3.0
...
c
     ...
     >   r,rp,rs,rb,zb,cw,rhow,attnw,cb,rhob,attn,alpw,alpb,ksq,ksqw,
     ...
     >   rb(mr),zb(mr),attn(mz),cb(mz),rhob(mz),cw(mz),
     >   f1(mz),f2(mz),f3(mz),alpw(mz),alpb(mz),rhow(mz),attnw(mz)
c
...
c
c       where psi3D = psi3D_1 - psi3D_2  (difference of source and image_source)
c
c p(z) will be evaluated along the z_grid at a range x1 for a point source at x0,zs.
c
